...
- **CPU / WASM target** - `compileWasm`/`compileWasmFn` compile the same `Fn` to a real WebAssembly module instead — real `i32`/`f64` types, no `eval`, and a `.batch(ctx, width, height)` that evaluates a whole pixel grid through one exported WASM function rather than one host call per pixel. Satisfies the same `CpuRoutine` interface as `compileJS`'s result, so code can pick between them without knowing which it got
