...
- `float` is `f64`, matching `compileJSRoutine`'s own JS-number arithmetic bit for bit — including the transcendental functions, which both backends call through the literal same `Math` object. Comparing a WASM result against a JS one needs no tolerance; comparing either against a GPU backend's `f32` result does (see [Caveats](compilation.md#caveats) on the JS target's own page).
- `isamplerCube`/`usamplerCube` are not supported yet — `compileJSRoutine` doesn't support them either.
