...
- `system.ts` — the one system this demo has: integrate position by
  ...
  and that single graph is what `compileJSRoutine`, `compileWasmRoutine` and
  ...
  `compileJSRoutine`/`compileWasmRoutine` call the same compiled function
  once per entity with `index` set to that entity's position (see
  `main.ts`'s `stepCPU`).
